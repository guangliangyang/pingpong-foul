...
import os
from pyswarm import pso  # 确保安装了 pyswarm
...
# Define threshold and detect anomalies with PSO optimization
...
# Objective function for PSO, returns negative F1 score for maximization
f1_progress = []  # Track F1 score progress for each iteration
positions_progress = []  # Track particle positions (threshold values) for each iteration

def f1_objective(threshold):
    current_threshold = calculate_threshold(model_baseline, normal_tensor, threshold)
    y_pred = detect_anomalies(model_baseline, test_tensor, current_threshold)
    f1 = f1_score(y_test_binary, y_pred, pos_label=1)
    f1_progress.append(f1)  # Track F1 score
    positions_progress.append(threshold)  # Track particle position (threshold)
...
lower_bound = 90.0  # Lower limit for threshold percentile
upper_bound = 99.0  # Upper limit for threshold percentile

# Run PSO
best_threshold, _ = pso(f1_objective, [lower_bound], [upper_bound], swarmsize=10, maxiter=20)

# Use the optimized threshold
optimal_threshold = calculate_threshold(model_baseline, normal_tensor, best_threshold)
y_pred_optimized = detect_anomalies(model_baseline, test_tensor, optimal_threshold)

# Evaluate results with optimized threshold
accuracy_optimized = np.mean(y_pred_optimized == y_test_binary)
f1_class_1 = f1_score(y_test_binary, y_pred_optimized, pos_label=1)

# Print results
print(f'Optimized Threshold Percentile: {best_threshold}')
print(f'Optimized Threshold Value: {optimal_threshold}')
...
# Plot PSO optimization progress - F1 score
plt.figure(figsize=(10, 6))
plt.plot(f1_progress, label='F1 Score Progress')
plt.title('PSO Optimization Progress for F1 Score')
