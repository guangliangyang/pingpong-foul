...
def draw_action_segmentation(frame_index, game, screen, screen_width):
    ...
    # Display the latest throw, highest, and hit frames as images under the legends
    if game.action_segments:
        latest_segment_index = len(game.action_segments) - 1
        latest_segment = game.action_segments[latest_segment_index]
        throw_frame = latest_segment['throw_frame']
        highest_frame = latest_segment['highest_frame']
        hit_frame = latest_segment['hit_frame']

        def get_frame_image(frame_number, cap):
            cap.set(cv2.CAP_PROP_POS_FRAMES, frame_number)
            ret, frame = cap.read()
            if ret:
                frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                frame_surface = pygame.surfarray.make_surface(cv2.resize(frame_rgb, (213, 160)).swapaxes(0, 1))
                return frame_surface
            return None

        throw_img = get_frame_image(throw_frame, game.caps['camera1'])
        highest_img = get_frame_image(highest_frame, game.caps['camera1'])
        hit_img = get_frame_image(hit_frame, game.caps['camera1'])

        # Define the image positions under the legend
        image_y = legend_y + 30
        label_font = pygame.font.SysFont("Arial", 14)  # Font for labels on images
        if throw_img:
