...
VIDEO_SCALE = 2
VIDEO_WIDTH = 640.0 * VIDEO_SCALE
VIDEO_HEIGHT = 480.0 * VIDEO_SCALE
ACTION_3D_Y_SPLIT = 0.4

SCREEN_WIDTH, SCREEN_HEIGHT = 2530, 1230
    ...
    def get_cached_frame_image(self, frame_index, frame_cache):
        for cached_index, frame in frame_cache:
            if cached_index == frame_index:
                frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                return pygame.surfarray.make_surface(cv2.resize(frame_rgb, (int(VIDEO_WIDTH/3) , int(VIDEO_HEIGHT/3))).swapaxes(0, 1))
        return None
...
def main():
    ...
    while running:
        ...
        data_panel_y = bar_y + bar_height + VIDEO_HEIGHT*2
...
def draw_camera_title(game, screen, screen_width):
    ...
    label_font = pygame.font.SysFont("Arial", 20*VIDEO_SCALE)
    ...
    screen.blit(left_label, (screen_width - game.VIDEO_WIDTH * VIDEO_SCALE + 10, 10))  # Left top corner of left camera
...
def draw_action_segmentation(frame_index, game, screen, screen_width):
    ...
    for segment in game.action_segments:
        ...
        pygame.draw.line(screen, (255, 255, 0), (throw_x, bar_y), (throw_x, bar_y + bar_height), 2 * VIDEO_SCALE)
        pygame.draw.line(screen, (255, 0, 0), (highest_x, bar_y), (highest_x, bar_y + bar_height), 2 * VIDEO_SCALE)
        pygame.draw.line(screen, (0, 255, 0), (hit_x, bar_y), (hit_x, bar_y + bar_height), 2 * VIDEO_SCALE)

    current_x = bar_x + ((frame_index - min_frame_index) / (max_frame_index - min_frame_index)) * bar_width
    pygame.draw.line(screen, (255, 255, 255), (current_x, bar_y), (current_x, bar_y + bar_height), 1 * VIDEO_SCALE)

    # Add legend under the bar
    legend_font = pygame.font.SysFont("Arial", 16 * VIDEO_SCALE)
    legend_x, legend_y = bar_x+10 , bar_y + bar_height + 10

    pygame.draw.rect(screen, (255, 255, 255), (legend_x, legend_y, 20* VIDEO_SCALE, 10* VIDEO_SCALE))
    screen.blit(legend_font.render("No Foul", True, (255, 255, 255)), (legend_x + 25* VIDEO_SCALE, legend_y - 5* VIDEO_SCALE))
    pygame.draw.rect(screen, (0, 0, 255), (legend_x + 100* VIDEO_SCALE, legend_y, 20* VIDEO_SCALE, 10* VIDEO_SCALE))
    screen.blit(legend_font.render("Foul", True, (255, 255, 255)), (legend_x + 125* VIDEO_SCALE, legend_y - 5* VIDEO_SCALE))

    pygame.draw.line(screen, (255, 255, 0), (legend_x + 200* VIDEO_SCALE, legend_y + 5* VIDEO_SCALE), (legend_x + 220* VIDEO_SCALE, legend_y + 5* VIDEO_SCALE), 2* VIDEO_SCALE)
    screen.blit(legend_font.render("Throw Point", True, (255, 255, 255)), (legend_x + 225* VIDEO_SCALE, legend_y - 5* VIDEO_SCALE))
    pygame.draw.line(screen, (255, 0, 0), (legend_x + 350* VIDEO_SCALE, legend_y + 5* VIDEO_SCALE), (legend_x + 370* VIDEO_SCALE, legend_y + 5* VIDEO_SCALE), 2* VIDEO_SCALE)
    screen.blit(legend_font.render("Highest Point", True, (255, 255, 255)), (legend_x + 375* VIDEO_SCALE, legend_y - 5* VIDEO_SCALE))
    pygame.draw.line(screen, (0, 255, 0), (legend_x + 500* VIDEO_SCALE, legend_y + 5* VIDEO_SCALE), (legend_x + 520* VIDEO_SCALE, legend_y + 5* VIDEO_SCALE), 2* VIDEO_SCALE)
    screen.blit(legend_font.render("Hit Point", True, (255, 255, 255)), (legend_x + 525* VIDEO_SCALE, legend_y - 5* VIDEO_SCALE))
    ...
    if game.action_segments:
        ...
        image_y = legend_y + 15 *  VIDEO_SCALE
        label_font = pygame.font.SysFont("Arial", 14* VIDEO_SCALE)
        ...
        if highest_img_cam1:
            screen.blit(highest_img_cam1, (legend_x + int(VIDEO_WIDTH/3) , image_y))
            screen.blit(label_font.render(f"{latest_segment_index} Cam1 Highest", True, (255, 255, 255)), (legend_x + int(VIDEO_WIDTH/3) , image_y))
        if hit_img_cam1:
            screen.blit(hit_img_cam1, (legend_x + int(VIDEO_WIDTH/3)*2, image_y))
            screen.blit(label_font.render(f"{latest_segment_index} Cam1 Hit", True, (255, 255, 255)), (legend_x + int(VIDEO_WIDTH/3)*2, image_y))

        # Display Camera 2 Images (Offset vertically)
        image_y_cam2 = image_y + int(VIDEO_HEIGHT/3) + 10  # Adjust vertical position for Camera 2 images
        if throw_img_cam2:
        ...
        if highest_img_cam2:
            screen.blit(highest_img_cam2, (legend_x + int(VIDEO_WIDTH/3), image_y_cam2))
            screen.blit(label_font.render(f"{latest_segment_index} Cam2 Highest", True, (255, 255, 255)), (legend_x + int(VIDEO_WIDTH/3) , image_y_cam2))
        if hit_img_cam2:
            screen.blit(hit_img_cam2, (legend_x + int(VIDEO_WIDTH/3)*2, image_y_cam2))
            screen.blit(label_font.render(f"{latest_segment_index} Cam2 Hit", True, (255, 255, 255)), (legend_x + int(VIDEO_WIDTH/3)*2, image_y_cam2))
