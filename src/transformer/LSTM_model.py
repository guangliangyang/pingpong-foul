...
# Dataset class that uses a sliding window
    ...
    def prepare_data(self):
        for traj_data in self.data:
            ...
            for i in range(HALF_WINDOW, len(sequence) - HALF_WINDOW):
                ...
                window_features = [point[:3] for point in window]  # Use only (x, y, z) for each point
                self.windows.append(window_features)
...
class TrajectoryLSTM(nn.Module):
    def __init__(self, input_dim=3, hidden_dim=64, num_classes=4, num_layers=1):
        super(TrajectoryLSTM, self).__init__()
        self.lstm = nn.LSTM(input_dim, hidden_dim, num_layers=num_layers, batch_first=True)
        self.classifier = nn.Linear(hidden_dim, num_classes)

    def forward(self, x):
        _, (hn, _) = self.lstm(x)  # x shape: (batch_size, seq_len, input_dim)
        x = self.classifier(hn[-1])  # Only take the last hidden state
        return x
...
model_file_path = "trained_trajectory_lstm_window.pth"
...
def predict_key_points(model, trajectory_data):
    ...
    for traj_data in trajectory_data:
        ...
        for i in range(HALF_WINDOW, len(sequence) - HALF_WINDOW):
            window = sequence[i - HALF_WINDOW: i + HALF_WINDOW + 1]
            window_features = [point[:3] for point in window]
            window_tensor = torch.tensor(window_features, dtype=torch.float32).unsqueeze(0)
